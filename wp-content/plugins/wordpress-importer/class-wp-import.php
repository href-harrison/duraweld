...
<?php if ( $this->allow_fetch_attachments() ) : ?>
	...
	public function get_author_mapping() {
		...
		foreach ( (array) $_POST['imported_authors'] as $i => $old_login ) {
			...
			if ( ! empty( $_POST['user_map'][ $i ] ) ) {
				...
				$existing_user = get_user_by( 'login', $old_login );
				if ( ! $existing_user && ! empty( $_POST['user_new'][ $i ] ) ) {
					$existing_user = get_user_by( 'login', $_POST['user_new'][ $i ] );
				}
				
				if ( $existing_user ) {
					// User already exists, use it
					if ( $old_id ) {
						$this->processed_authors[ $old_id ] = $existing_user->ID;
					}
					$this->author_mapping[ $santized_old_login ] = $existing_user->ID;
				} else {
					...
					if ( ! is_wp_error( $user_id ) ) {
						...
						// If creation failed, check again if user exists (race condition)
						$error_code = $user_id->get_error_code();
						if ( ( $error_code === 'existing_user_login' || $error_code === 'existing_user_email' ) ) {
							$existing_user = get_user_by( 'login', $old_login );
							if ( ! $existing_user && ! empty( $user_data['user_email'] ) ) {
								$existing_user = get_user_by( 'email', $user_data['user_email'] );
							}
							
							if ( $existing_user ) {
								// Use existing user instead of showing error
								if ( $old_id ) {
									$this->processed_authors[ $old_id ] = $existing_user->ID;
								}
								$this->author_mapping[ $santized_old_login ] = $existing_user->ID;
	...
	public function fetch_remote_file( $url, $post ) {
		...
		// Add authentication if credentials are defined
		...
		if ( defined( 'PRIMA_IMPORT_USER' ) && defined( 'PRIMA_IMPORT_PASS' ) ) {
			$headers['Authorization'] = 'Basic ' . base64_encode( PRIMA_IMPORT_USER . ':' . PRIMA_IMPORT_PASS );
		} elseif ( defined( 'PRIMA_IMPORT_TOKEN' ) ) {
			$headers['Authorization'] = 'Bearer ' . PRIMA_IMPORT_TOKEN;
		}
