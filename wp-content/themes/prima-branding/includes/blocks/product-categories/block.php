...
$category_items = get_field('category_items') ?: false;

// If that didn't work and we have a post_id, try with post context
if (!$category_items && isset($post_id)) {
	$category_items = get_field('category_items', $post_id) ?: false;
}

// If block data exists and is not just a number (row count), use it
if (!$category_items && isset($block['data']['category_items'])) {
	$raw_data = $block['data']['category_items'];
	// Only use if it's actual array data, not just a count
	if (is_array($raw_data) && !is_numeric(key($raw_data))) {
		$category_items = $raw_data;
	}
}

// Ensure category_items is an array of arrays (repeater format)
if ($category_items) {
	...
	if (!is_array($category_items)) {
		...
		$category_items = array_filter($category_items, function($item) {
			// Item should be an array with sub-fields (image, heading, link)
...
} elseif ($is_preview && empty($category_items)) {
	?>
	<div class="categories-empty">
		<p><strong>Product Categories Block</strong></p>
		<p>Add category items using the repeater field in the sidebar.<br>Recommended: 4 items for optimal layout.</p>
		<div class="categories-preview-grid">
			<?php for($i = 1; $i <= 4; $i++) : ?>
				<div class="categories-preview-item">
					<div class="categories-preview-image">
						<span>Image <?php echo $i; ?></span>
					</div>
					<div class="categories-preview-label">Category <?php echo $i; ?></div>
				</div>
			<?php endfor; ?>
		</div>
	</div>
